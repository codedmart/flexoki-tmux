...
```bash
set -g @flexoki-tmux_show_datetime 1   # 1 = enabled (default) | 0 = disabled
set -g @flexoki-tmux_date_format YMD   # see options below
set -g @flexoki-tmux_time_format 24H   # see options below
```
...
```bash
set -g @flexoki-tmux_show_music 1
```
...
```bash
set -g @flexoki-tmux_show_netspeed 1
set -g @flexoki-tmux_netspeed_iface "wlan0"  # auto-detected via default route if omitted
set -g @flexoki-tmux_netspeed_showip 1       # show IPv4 address (default: 0)
set -g @flexoki-tmux_netspeed_refresh 1      # update interval in seconds (default: 1)
```

**Requirements:** [bc](https://www.gnu.org/software/bc/)

The interface is auto-detected from the default route if `@flexoki-tmux_netspeed_iface` is not set.
...
```bash
set -g @flexoki-tmux_show_battery_widget 1
set -g @flexoki-tmux_battery_name "BAT1"      # default: 'BAT1' (Linux), 'InternalBattery-0' (macOS)
set -g @flexoki-tmux_battery_low_threshold 21  # show warning below this % (default: 21)
```
...
```bash
set -g @flexoki-tmux_show_hostname 1
```
