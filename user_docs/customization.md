...
Flexoki Tmux uses stylized numbers for window IDs, pane IDs, and the zoom indicator. Each can be configured independently.

```bash
set -g @flexoki-tmux_window_id_style digital   # window tab numbers
set -g @flexoki-tmux_pane_id_style hsquare     # pane number shown in status bar
set -g @flexoki-tmux_zoom_id_style dsquare     # pane number shown when zoomed
```
...
```bash
set -g @flexoki-tmux_terminal_icon        # icon for inactive windows (default: )
set -g @flexoki-tmux_active_terminal_icon  # icon for the active window (default: )
```
...
```bash
set -g @flexoki-tmux_window_tidy_icons 1   # 1 = no extra space | 0 = add space (default)
```
...
```bash
set -g @flexoki-tmux_zoom_id_style dsquare
```
