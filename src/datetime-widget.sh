...
# Check if enabled
ENABLED=$(tmux show-option -gv @flexoki-tmux_show_datetime 2>/dev/null)
[[ ${ENABLED} -ne 1 ]] && exit 0
...
date_format=$(tmux show-option -gv @flexoki-tmux_date_format 2>/dev/null)
time_format=$(tmux show-option -gv @flexoki-tmux_time_format 2>/dev/null)
