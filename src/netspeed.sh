...
# Check if enabled
ENABLED=$(tmux show-option -gv @flexoki-tmux_show_netspeed 2>/dev/null)
[[ ${ENABLED} -ne 1 ]] && exit 0
...
INTERFACE=$(tmux show-option -gv @flexoki-tmux_netspeed_iface 2>/dev/null)
# Show IP address
SHOW_IP=$(tmux show-option -gv @flexoki-tmux_netspeed_showip 2>/dev/null)
# Time between refresh
TIME_DIFF=$(tmux show-option -gv @flexoki-tmux_netspeed_refresh 2>/dev/null)
...
if [[ -z $INTERFACE ]]; then
  ...
  tmux set-option -g @flexoki-tmux_netspeed_iface "$INTERFACE"
fi
