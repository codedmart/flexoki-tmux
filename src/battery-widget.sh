...
ENABLED=$(tmux show-option -gv @flexoki-tmux_show_battery_widget 2>/dev/null)
[[ ${ENABLED} -ne 1 ]] && exit 0
...
BATTERY_NAME=$(tmux show-option -gv @flexoki-tmux_battery_name 2>/dev/null)
BATTERY_LOW=$(tmux show-option -gv @flexoki-tmux_battery_low_threshold 2>/dev/null)
